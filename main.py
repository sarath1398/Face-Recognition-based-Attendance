...
df=pd.DataFrame({'Name':[names[:-4] for names in db_list],'Status':['Absent' for n in range(len(db_list))]})
...
def predict_function(face):
    ...
    for db_embedded in database_embeddings:
        ...
        if e_distance < 1.01 and c_similarity>0.6:

            max_similarity = c_similarity
            min_distance = e_distance[0]
            index = count
...
while time.time()<webcam_end:
    ...
    faces = detector.detectMultiScale(gray, 1.1,8,minSize=(100,100))
